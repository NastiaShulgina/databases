...
public class ViewOperations<T, K extends Serializable> {
    ...
    private void inputValueForField(Field field, T entity) {
        ...
        while (true) {
            ...
            try {
                ...
                } else if (fieldType == Problem.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Problem, Integer> responseService = new ProblemServiceImpl();
                    field.set(entity, responseService.findById(id));
                } else if (fieldType == Application.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Application, Integer> clusterProgramService = new ApplicationServiceImpl();
                    field.set(entity, clusterProgramService.findById(id));
                } else if (fieldType == ResponsibleWorker.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<ResponsibleWorker, Integer> speakerService = new ResponsibleWorkerServiceImpl();
                    field.set(entity, speakerService.findById(id));
                } else if (fieldType == Solution.class) {
                    AbstractService<Solution, String> studentService = new SolutionServiceImpl();
                    field.set(entity, studentService.findById(inputText));
                } else if (fieldType == Location.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Location, Integer> lecturerService = new LocationServiceImpl();
    ...
    private void getUserHintForField(String fieldName, Class<?> fieldType) {
        String hint = null;
        if (fieldType == Integer.class || fieldType == Problem.class || fieldType == Application.class
                || fieldType == Location.class || fieldType == ResponsibleWorker.class) {
            hint = "integer using [0-9] digits";
        } else if (fieldType == String.class || fieldType == Solution.class) {
